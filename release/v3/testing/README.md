# MiniHarnessCode v3 真人场景测试包

这个目录把 MiniHarnessCode v3 相对 `main` 的真人使用场景测试整理成一个可以复盘、重跑、继续扩展的测试包。

它不是单元测试说明，也不是 pytest 清单。它记录的是：像真实用户一样从 `uv run MiniHarnessCode`、`--repl`、slash command、resume、provider profile、skills、worker、memory 等入口使用 MiniHarnessCode，然后只读检查 `.pico/runs` 和 `.pico/sessions` 产物。
...
指定输出目录时必须放在 MiniHarnessCode repo 外：
...
- 场景必须从用户入口驱动 MiniHarnessCode，不能 import `Pico` 直接调 runtime。
- 验证器可以读文件，但只能读 MiniHarnessCode 自己写出的 artifacts 和 scenario workspace。
- 输出目录必须在 repo 外，避免 MiniHarnessCode 向上发现真实 repo root。
