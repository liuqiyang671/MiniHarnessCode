...
def test_explore_agent_runs_real_readonly_child_session_and_records_notification(tmp_path):
    agent = build_agent(
        tmp_path,
        [
    ...
    notifications = [item for item in agent.session["history"] if item["role"] == "user" and "<task-notification>" in item["content"]]
    ...
    assert any(event["event"] == "worker_started" and event["worker_id"] == "agent_1" for event in events)
    assert any(event["event"] == "worker_finished" and event["worker_id"] == "agent_1" for event in events)
...
def test_worker_agent_can_be_continued_with_same_child_context_and_write_scope(tmp_path):
    agent = build_agent(
        tmp_path,
        [
    ...
    assert all("<task-id>agent_1</task-id>" in item["content"] for item in notifications)
    events = read_jsonl(agent.session_event_bus.path)
    assert sum(1 for event in events if event["event"] == "worker_started" and event["worker_id"] == "agent_1") == 2
...
def test_worker_write_scope_blocks_child_file_modification_outside_scope(tmp_path):
    ...
    notification = next(item for item in agent.session["history"] if item["role"] == "user" and "<task-notification>" in item["content"])
...
def test_plan_mode_cannot_continue_write_capable_worker(tmp_path):
    ...
    rejected = agent.run_tool("send_message", {"to": "agent_1", "message": "Write notes/out.txt"})
...
def test_plan_mode_allows_only_explore_agents(tmp_path):
    ...
    rejected = agent.run_tool(
        "agent",
        {"description": "Write from plan", "prompt": "change files", "subagent_type": "worker", "write_scope": ["pico"]},
    )
